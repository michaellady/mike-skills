...
| **Architecture conformance** | Dependency cycles, layer/boundary violations, illegal imports the complexity gates don't see | A layered/modular codebase. Encode structural rules as executable tests (ArchUnit, import-linter, dependency-cruiser). |
...
| **Crash-recovery / durability** | Corruption or lost acknowledged writes after a hard crash — committed state must survive + recover | Code that persists state (DB/WAL/file formats/stateful services). Kill mid-write/mid-fsync, restart, assert no corruption + correct recovery. The persistence sibling of chaos (degradation vs durability). |
