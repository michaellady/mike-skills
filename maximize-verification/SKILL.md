...
This is where verification is *maximized*. Each layer below is a distinct failure-mode lens — it catches a class of bug the others structurally cannot. Turn on **every layer that applies to this code**; they are additive, never alternatives. Walk the table top to bottom and for each one decide *applies / doesn't apply / why*, so the output is an explicit maximal stack rather than a single technique. (The table order is a walkthrough checklist, not a priority — once you have the "on" set, order it by *leverage* per the note after the table.)
...
| **Fuzzing** | Crashes, panics, hangs, UB, and unhandled inputs across the whole input space | Code parses/decodes structured or untrusted input, or has a large input surface. |
...
| **Static verification** | Whole bug *classes* eliminated without executing — type errors, non-exhaustive matches, nullability, lint violations, proved properties (TLA+/Alloy) | Always. The cheapest, most uncheatable signal; turn the compiler/linter into a gate with `-Werror`. |
...
| **Mutation testing** (Step 5) | Whether the stack you just built would actually *catch* a bug — measures the verification, not the code | Run last, to validate the whole stack and produce the number you report. |

Maximal ≠ exhaustive busywork. Prioritize by leverage (oracle → static → property/metamorphic → the nature-specific layers → review → mutation) and stop adding layers when the next one's *independent* signal isn't worth its cost. The goal is to maximize uncorrelated coverage of the failure space, not to run the longest possible tool list.
...
Assemble `$ASSEMBLED_PROMPT` yourself using adversarial-review's Phase 2 scaffold — inline the `source_content`, the `rules_list`, and the `drafts` as a numbered list into one prompt string (the binary owns transport + merge only; composing the prompt is the caller's job). Then invoke (build once if the binary is missing):
...
## Step 5 — Coverage reporting (the trap)

Coverage measures what was **executed**, not what was **verified**. A test with zero assertions that just calls the code gives 100% line coverage. So:

- **Report the mutation score, not line coverage.** Mutation testing injects bugs and checks whether the suite catches them — it measures whether your tests would *catch* a defect, which is what you actually care about. If you report one number for an agent-generated suite, make it the mutation score.
- Recommend the language-matched mutation tool: Stryker (JS/TS), mutmut (Python), PIT (Java), cargo-mutants (Rust). See [REFERENCE.md](./REFERENCE.md) for the full table.
...
3. **The stack** — walk the Step 2 layer list and mark each *on / off / why*: differential, property, metamorphic, fuzzing, concurrency, static, performance, chaos. The "on" set, ordered by leverage, IS the maximal harness.
...
6. **Metric to report** — mutation score (with the tool name); never line coverage alone.
